...
def inject_test_elections(cls, argv):  # pragma: no cover
    ...
    def make_test_runner(election_path, output_path):
        def run_test(self):
            ...
            if (
                smushed_and_lowered_expected != smushed_and_lowered_got
            ):  # pragma: no cover
                self.maxDiff = 2 ** 32
                self.assertEqual("\n".join(expected) + "\n", "\n".join(got) + "\n")
    ...
    #
    # If you want to change options for ed .csv file,
    # create ed .starvote file with the same basename
    ...
    if not work:  # pragma: no cover
        ...
        for extension in test_suffixes:
            for election_path in test_directory.glob(f"*{extension}"):
                ...
                if already_seen_as:
                    print(
                        f"Skipping '{election_path}', already processed '{already_seen_as}'"
                    )
                    continue
    ...
    for i, (election_path, output_path) in enumerate(work, 1):
        ...
        runner_name = "test_elections__slash__" + str(election_path).rpartition("/")[
            2
        ].replace(".", "_dot_")
...
class StarvoteTests(unittest.TestCase):
    def test_import_star(self):
        ...
        # (it will fail if there's ed bug with the definition of __all__)
    ...
    def test_random_tiebreaker(self):
        election = tied_election.format(
            options="tiebreaker = on_demand_random_tiebreaker"
        )
        ...
        self.assertIn(result[0], ["Cab", "B", "C"])
        output = text_getvalue()
        self.assertIn(
            "Tie-breaking winners will be chosen at random, on demand.", output
        )
    ...
    def test_hardcoded_candidates_tiebreaker_with_description(self):
        ...
        kwargs["tiebreaker"] = starvote.predefined_permutation_tiebreaker(
            candidates="C B Cab".split(), description=description_string
        )
        text_clear, text_print, text_getvalue = starvote._printer()
        result = starvote.election(**kwargs, print=text_print)
        self.assertEqual(result, ["C"])
    ...
    def test_options(self):
        ...
        with self.assertRaises(ValueError):
            o.break_tie(text="foo", candidates=["ed", "b"], desired=3)

        o = starvote.Options(
            method=starvote.STAR_Voting,
            verbosity=1,
            tiebreaker=lambda options, tie, desired, exception: 3.1415,
        )
        with self.assertRaises(TypeError):
            o.break_tie(text="foo", candidates=["ed", "b"], desired=1)

        o = starvote.Options(
            method=starvote.STAR_Voting,
            verbosity=1,
            tiebreaker=lambda options, tie, desired, exception: [
                "ed",
                "b",
                "c",
                "d",
    ...
    def test_invalid_ballots(self):
        ...
        with self.assertRaises(ValueError):
            starvote.star_voting([{"Cab": 80, "B": 90}])
        with self.assertRaises(ValueError):
            starvote.star_voting([{"Cab": 1.5, "B": 3.5}])
    ...
    def test_module_main(self):
        ...
        result = main(
            f"-s 2 -x 5 -m rrv -v -v -t none -- {good_starvote_file}".split(),
            print=text_print,
        )
    ...
    def test_int_to_words(self):
        ...
        test(-1, "negative one", "negative one")
        ...
        test(11, "eleven", "eleven")

        test(12, "twelve", "twelve")

        test(13, "thirteen", "thirteen")

        test(14, "fourteen", "fourteen")

        test(15, "fifteen", "fifteen")

        test(16, "sixteen", "sixteen")

        test(17, "seventeen", "seventeen")

        test(18, "eighteen", "eighteen")

        test(19, "nineteen", "nineteen")

        test(20, "twenty", "twenty")

        test(21, "twenty-one", "twenty-one")

        test(22, "twenty-two", "twenty-two")

        test(23, "twenty-three", "twenty-three")

        test(24, "twenty-four", "twenty-four")

        test(25, "twenty-five", "twenty-five")

        test(26, "twenty-six", "twenty-six")

        test(27, "twenty-seven", "twenty-seven")

        test(28, "twenty-eight", "twenty-eight")

        test(29, "twenty-nine", "twenty-nine")

        test(30, "thirty", "thirty")

        test(40, "forty", "forty")

        test(41, "forty-one", "forty-one")

        test(42, "forty-two", "forty-two")

        test(50, "fifty", "fifty")

        test(51, "fifty-one", "fifty-one")

        test(52, "fifty-two", "fifty-two")

        test(60, "sixty", "sixty")

        test(61, "sixty-one", "sixty-one")

        test(62, "sixty-two", "sixty-two")

        test(70, "seventy", "seventy")

        test(71, "seventy-one", "seventy-one")

        test(72, "seventy-two", "seventy-two")

        test(80, "eighty", "eighty")

        test(81, "eighty-one", "eighty-one")

        test(82, "eighty-two", "eighty-two")

        test(90, "ninety", "ninety")

        test(91, "ninety-one", "ninety-one")

        test(92, "ninety-two", "ninety-two")

        test(100, "one hundred", "one hundred")

        test(101, "one hundred one", "one hundred and one")

        test(102, "one hundred two", "one hundred and two")

        test(200, "two hundred", "two hundred")

        test(201, "two hundred one", "two hundred and one")

        test(202, "two hundred two", "two hundred and two")

        test(211, "two hundred eleven", "two hundred and eleven")

        test(222, "two hundred twenty-two", "two hundred and twenty-two")

        test(300, "three hundred", "three hundred")

        test(301, "three hundred one", "three hundred and one")

        test(302, "three hundred two", "three hundred and two")

        test(311, "three hundred eleven", "three hundred and eleven")

        test(322, "three hundred twenty-two", "three hundred and twenty-two")

        test(400, "four hundred", "four hundred")

        test(401, "four hundred one", "four hundred and one")

        test(402, "four hundred two", "four hundred and two")

        test(411, "four hundred eleven", "four hundred and eleven")

        test(422, "four hundred twenty-two", "four hundred and twenty-two")

        test(500, "five hundred", "five hundred")

        test(501, "five hundred one", "five hundred and one")

        test(502, "five hundred two", "five hundred and two")

        test(511, "five hundred eleven", "five hundred and eleven")

        test(522, "five hundred twenty-two", "five hundred and twenty-two")

        test(600, "six hundred", "six hundred")

        test(601, "six hundred one", "six hundred and one")

        test(602, "six hundred two", "six hundred and two")

        test(611, "six hundred eleven", "six hundred and eleven")

        test(622, "six hundred twenty-two", "six hundred and twenty-two")

        test(700, "seven hundred", "seven hundred")

        test(701, "seven hundred one", "seven hundred and one")

        test(702, "seven hundred two", "seven hundred and two")

        test(711, "seven hundred eleven", "seven hundred and eleven")

        test(722, "seven hundred twenty-two", "seven hundred and twenty-two")

        test(800, "eight hundred", "eight hundred")

        test(801, "eight hundred one", "eight hundred and one")

        test(802, "eight hundred two", "eight hundred and two")

        test(811, "eight hundred eleven", "eight hundred and eleven")

        test(822, "eight hundred twenty-two", "eight hundred and twenty-two")

        test(900, "nine hundred", "nine hundred")

        test(901, "nine hundred one", "nine hundred and one")

        test(902, "nine hundred two", "nine hundred and two")

        test(911, "nine hundred eleven", "nine hundred and eleven")

        test(922, "nine hundred twenty-two", "nine hundred and twenty-two")

        test(1000, "one thousand", "one thousand")

        test(1001, "one thousand one", "one thousand and one")

        test(1002, "one thousand two", "one thousand and two")

        test(1023, "one thousand twenty-three", "one thousand and twenty-three")

        test(1034, "one thousand thirty-four", "one thousand and thirty-four")
        ...
        test(2000, "two thousand", "two thousand")

        test(2001, "two thousand one", "two thousand and one")

        test(2002, "two thousand two", "two thousand and two")

        test(2023, "two thousand twenty-three", "two thousand and twenty-three")

        test(2034, "two thousand thirty-four", "two thousand and thirty-four")
        ...
        test(3000, "three thousand", "three thousand")

        test(3001, "three thousand one", "three thousand and one")

        test(3002, "three thousand two", "three thousand and two")

        test(3023, "three thousand twenty-three", "three thousand and twenty-three")

        test(3034, "three thousand thirty-four", "three thousand and thirty-four")
        ...
        test(4000, "four thousand", "four thousand")

        test(4001, "four thousand one", "four thousand and one")

        test(4002, "four thousand two", "four thousand and two")

        test(4023, "four thousand twenty-three", "four thousand and twenty-three")

        test(4034, "four thousand thirty-four", "four thousand and thirty-four")
        ...
        test(5000, "five thousand", "five thousand")

        test(5001, "five thousand one", "five thousand and one")

        test(5002, "five thousand two", "five thousand and two")

        test(5023, "five thousand twenty-three", "five thousand and twenty-three")

        test(5034, "five thousand thirty-four", "five thousand and thirty-four")
        ...
        test(6000, "six thousand", "six thousand")

        test(6001, "six thousand one", "six thousand and one")

        test(6002, "six thousand two", "six thousand and two")

        test(6023, "six thousand twenty-three", "six thousand and twenty-three")

        test(6034, "six thousand thirty-four", "six thousand and thirty-four")
        ...
        test(7000, "seven thousand", "seven thousand")

        test(7001, "seven thousand one", "seven thousand and one")

        test(7002, "seven thousand two", "seven thousand and two")

        test(7023, "seven thousand twenty-three", "seven thousand and twenty-three")

        test(7034, "seven thousand thirty-four", "seven thousand and thirty-four")
        ...
        test(8000, "eight thousand", "eight thousand")

        test(8001, "eight thousand one", "eight thousand and one")

        test(8002, "eight thousand two", "eight thousand and two")

        test(8023, "eight thousand twenty-three", "eight thousand and twenty-three")

        test(8034, "eight thousand thirty-four", "eight thousand and thirty-four")
        ...
        test(9000, "nine thousand", "nine thousand")

        test(9001, "nine thousand one", "nine thousand and one")

        test(9002, "nine thousand two", "nine thousand and two")

        test(9023, "nine thousand twenty-three", "nine thousand and twenty-three")

        test(9034, "nine thousand thirty-four", "nine thousand and thirty-four")
        ...
        test(10000, "ten thousand", "ten thousand")

        test(10001, "ten thousand one", "ten thousand and one")

        test(10002, "ten thousand two", "ten thousand and two")

        test(10023, "ten thousand twenty-three", "ten thousand and twenty-three")

        test(10034, "ten thousand thirty-four", "ten thousand and thirty-four")
        ...
        test(11000, "eleven thousand", "eleven thousand")

        test(11001, "eleven thousand one", "eleven thousand and one")

        test(11002, "eleven thousand two", "eleven thousand and two")

        test(11023, "eleven thousand twenty-three", "eleven thousand and twenty-three")

        test(11034, "eleven thousand thirty-four", "eleven thousand and thirty-four")
